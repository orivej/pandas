        ...
        if _is_tzlocal(tz1):
            for i in range(n):
                ...
                if v == NPY_NAT:
                    ...
                    delta = (int(timedelta.total_seconds(_get_utcoffset(tz1, dt)))
                             * 1000000000)
                    utc_dates[i] = v - delta
        else:
    ...
    if _is_tzlocal(tz2):
        for i in range(n):
            ...
            if v == NPY_NAT:
                ...
                delta = int(timedelta.total_seconds(
...
def tz_convert_single(int64_t val, object tz1, object tz2):
    ...
    if _is_tzlocal(tz1):
        ...
        dt = datetime(dts.year, dts.month, dts.day, dts.hour,
                      dts.min, dts.sec, dts.us, tz1)
        delta = int(timedelta.total_seconds(_get_utcoffset(tz1, dt))) * 1000000000
        utc_date = val - delta
    ...
    if _is_tzlocal(tz2):
        ...
        dt = datetime(dts.year, dts.month, dts.day, dts.hour,
                      dts.min, dts.sec, dts.us, tz2)
        delta = int(timedelta.total_seconds(_get_utcoffset(tz2, dt))) * 1000000000
...
cdef object _get_dst_info(object tz):
    ...
    if cache_key is None:
        num = int(timedelta.total_seconds(_get_utcoffset(tz, None))) * 1000000000
...
cpdef ndarray _unbox_utcoffsets(object transinfo):
    ...
    for i in range(sz):
        arr[i] = int(timedelta.total_seconds(transinfo[i][0])) * 1000000000
